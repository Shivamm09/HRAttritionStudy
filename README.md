...
Employee Attrition is an increasingly important issue for companies as it is a source of large losses not only financial. I decided to analyze and understand what factors contribute to making an employee leave the company. Some of the features which contribute include stock options, their job role, age, marital status, monthly income among others. The aim was to create an ML model which would be able to look at the factors for each employee and determine whether they would leave the company. The best model produced was a linear support-vector machine, which produced a precision of 83% and recall of 43%. The metric used was precision to determine the perfomance of the model (while keeping a decent recall). The notebooks provide the code for my analysis and machine learning model, and my pdf file in the repository includes the slides summarizing the project.
...
The above is an example showing how the explainability of the model helps the HR team understand which factors are affecting the decision of the individual for leaving the company. The coefficient form the SVM model of a given feature is multiplie by the value of that feature renormalized to lie between 0 and 1 using the minmax method used in the feature engineering process.To account for the fact that some features are represented by negative values, I set the threshold for a feature making an employee leave at -0.3 instead of 0. The above is an example of an employee and their features.As you can see, the most concerning factors (in the red) are that they work overtime and they do a lot of business trips; however, the factors that are good are that they have a good work life balance and are fairly satisified with their job. Even though there are many factors which the model takes in which can help the HR team and which they can attempt to change, there are some factors which the model takes as inputs (gender, department,...). This means that some of these set values can potentially "tip the scales" for an employee to want to leave according to this model, but cannot be changed. It would be helpful knowing from which department and specific job roles are individuals likely to leave however, it mgiht be difficult to address that because it is what they have been employed for.
